...
let shower = false
let showText = document.getElementsByClassName('ba-txtsub')
let readMoreButton = document.getElementsByClassName('ba-button')
for(let i = 0; i < readMoreButton.length && i < showText.length; i++){
    readMoreButton[i].addEventListener('click', function(){
        if(shower === true){
            shower = false
            showText[i].style.height = '3.5rem'
        }
        else {
            shower = true
            showText[i].style.height = 'fit-content'
        }
    })
}
